                    ...
                    if (isEmpty) {
                        ...
                        if (Integer.class.isAssignableFrom(param.getClass())) {
                            ...
                            if (parameterValid.isSection()) {
                                ...
                                if (!(paramInt > section[INT_PARAM_SIZE_SUBSCRIPT_MIN]
                                        && paramInt < section[INT_PARAM_SIZE_SUBSCRIPT_MAX])) {
                                    errorLists.add(errorMsg);
                                } else if (!(paramInt > section[INT_PARAM_SIZE_SUBSCRIPT_MAX]
                                        && paramInt < section[INT_PARAM_SIZE_SUBSCRIPT_MIN])) {
                                    errorLists.add(errorMsg);
                                }
                            }
                        }
                        ...
                        if (String.class.isAssignableFrom(param.getClass())) {
                            boolean isInfrared = ((ParameterValid) object).isInfrared();
                            if (isInfrared) {
                                ...
                                if (parameterValid.isBlank()) {
                                    if (StringUtils.isBlank(paramStr)) {
                                        ...
                                        if (length == STRING_SIZE
                                                && STRING_EMPTY_DOUBLE_CHARACTER == begin
                                                && STRING_EMPTY_DOUBLE_CHARACTER == end) {
                                            errorLists.add(errorMsg);
                                        }
                                        if (length == STRING_SIZE
                                                && STRING_EMPTY_SINGLE_CHARACTER == begin
